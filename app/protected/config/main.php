<?php
  $config = parse_ini_file('/var/secure/filtered.ini', true);
  if ($config['env']<>'live') 
...
$options = array(
	...
	'preload'=>array(
	  ...
	  'mailstore', // yii helper library for mailgun store
	  'mimeparse',
	  'advanced'
	  ),
...
'user'=>array(            
  ...
  'authManager' => array(
	...
	'params'=>array(
		...
		'adminEmail'=>$config['support_email'],
		'supportEmail'=>$config['support_email'],
		'verify_secret'=>$config['verify_secret'],
		'scan_seconds'=> intval($config['scan_seconds']),
		'postsPerPage'=> intval($config['postsPerPage']),
    'mailgun'=> array(
    ...
    'pushover'=> array(
      'key'=> $config['pushover_key'],
		...
		'account_salt'=> $config['account_salt'],
...
if ($config['env'] <> 'live') {
  ...
  $options['components']['log']=
		array(
  			'class'=>'CLogRouter',
  			'routes'=>array(
          array('class'=>'CFileLogRoute','levels'=>'error, warning, info,trace')
    	  ));  
}
return $options;
