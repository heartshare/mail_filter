...
class FolderController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','index','view','delete','train'),
				'users'=>array('@'),
			),
    ...
    $model->digest =1;
      ...
      $model->modified_at =new CDbExpression('NOW()');          
			if($model->save()) {
			  $model->createRemote($model->account_id,$model->name);
		...
		if(isset($_POST['Folder']))
		{
			...
			if ($model->validate()) {
			  if ($current_name <> $model->name) {
			    $result = $model->renameRemote($model->account_id,$current_name,$model->name);
			  }
  			if($model->save()) {
  				$this->redirect(array('index'));
  			}			  
	...
	public function actionTrain($id)
	{
		...
		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);
			    $result = $model->train($id);
      		$this->render('train',array(
      			'model'=>$model,'train'=>$result,
      		));
	}
